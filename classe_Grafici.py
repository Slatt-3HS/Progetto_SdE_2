...
class Grafici:
    ...
    @staticmethod
    def crea_grafico_linea_old(dat,
                           x_col: str,
                           y_col: str,
                           color_col: str = None,
                           title: str = None,
    ...
    def crea_grafico_linea(dat,
                           ...
                           height: int = 400,
                           totale: bool = False,
                           label_angle: int = 0,  # Nuovo parametro per l'angolo delle etichette
                           show_legend: bool = True):  # Nuovo parametro per visualizzare/nascondere la legenda
        """
        Funzione avanzata per creare un grafico a linee con opzioni di highlight e interattività
        """
        base = alt.Chart(dat).mark_line(point=True).encode(
            ...
            base = base.encode(
                color=alt.Color(
                    ...
                    legend=alt.Legend(title=color_col.title(), orient='right',
                                      columnPadding=10) if show_legend else None  # Mostra/nascondi la legenda
                )
            )
        elif color_col == "Sex" and totale:
            base = base.encode(
    ...
    def crea_grafico_barre(dat,
                           ...
                           label_angle: int = 0):  # Nuovo parametro per l'inclinazione delle labels
        """
    ...
    @staticmethod
    def create_grafico_torta_old(dat, category_col: str, value_col: str, title: str = None,
    ...
    def create_grafico_torta(dat, category_col: str, value_col: str, title: str = None,
                             width: int = 600, height: int = 300, inner_radius: int = 50,
                             show_legend: bool = True):  # Nuovo parametro show_legend
        """
        grafico a torta
        """
        ...
        # Aggiungi il totale al centro (come valore numerico con separatore per migliaia)
        total_text = alt.Chart(pd.DataFrame([{'total': total}])).mark_text(
            ...
            text=alt.Text('total:Q', format=',.0f')  # Aggiunge il separatore delle migliaia
        ).properties(
            width=width,
            height=height
        )
